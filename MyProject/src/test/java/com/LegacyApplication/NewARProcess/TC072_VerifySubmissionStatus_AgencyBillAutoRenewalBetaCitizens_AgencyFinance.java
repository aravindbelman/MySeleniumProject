...
public class TC072_VerifySubmissionStatus_AgencyBillAutoRenewalBetaCitizens_AgencyFinance extends TestBase {
	
	public static final Logger log=Logger.getLogger(TC072_VerifySubmissionStatus_AgencyBillAutoRenewalBetaCitizens_AgencyFinance.class.getName());
	...
	public void review_submissionStatus_SpecialAgency_AgencyFinance() throws InterruptedException, IOException, AWTException
	{
		...
		HomePageMethods.login_as_admin();
		WaitMethods.wait_in_seconds(5);
	    AutoRenewalBrokerFeeMethods.verify_renewal_status("Renewal Submission Id", 2);
	    ...
	    AutoRenewalBrokerFeeMethods.verify_important_renewalInfo(2);
	    AutoRenewalBrokerFeeMethods.verify_renewalLetter_paymentInfo("agency finance", 1);
	    WaitMethods.wait_in_seconds(2);
			
		log.info("<===========Completed verifying submission status of special agency with AgencyFinance as Payment Option Test===========> ");
	}
